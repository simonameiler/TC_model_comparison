...
description: Impact analysis - post-processing; confidence intervals 
...
def main(reg):
    ...
    # functions to derive sampling error
    rng = np.random.default_rng(123456789)   
    
    # subsampling IBTrACS INCL. original record
    def draws_sampling_error_IBTrACS(yrs_hist=39):
        gen = list()
        for i in range(100):
    ...
    def save_results_CIs(imp_dict, N=1000, yrs_hist=1014, save='IBTrACS_p'):
        ...
        save_str = res_dir/f"rp_imps_longCIs_{reg}_{save}.npz"
        np.savez_compressed(
    ...
    def save_results_impact(imp_dict, N=1000, yrs_hist=1014, save='IBTrACS_p'):
        ...
        save_str = res_dir/f"impacts_longCIs_{reg}_{save}.npz"
        np.savez_compressed(
    ...
    ############################ Call functions ###############################
    # get subsamples
    ibtracs = draws_sampling_error_IBTrACS(yrs_hist=39)
    ...
    for n in range(1000):
        ...
        long_ibtracs['IB_long_'+str(n)] = concat_IB 
