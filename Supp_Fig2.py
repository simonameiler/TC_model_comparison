...
description: Impact analysis - confidence intervals for longer time periods
    building on the sampling error scheme in TC_imp_analysis.py
...
def load_results(reg,load='IBTrACS_p'):
    load_str = data_dir/f"rp_imps_longCIs_{reg}_{load}_norm.npz"
...
# load IBTrACS impact from csv and
impact_dict = dict()
ibtracs_rp = dict()
ibtracs_imp = dict()
for reg in basin_label.keys():
    imp_str = f"TC_{reg}_impact_IBTrACS_norm.csv"
...
for k, reg in enumerate(basin_label.keys()):
    ...
    axis[basin_axes[reg][0],basin_axes[reg][1]].set_ylim(1e-6, 2e-2)
...
fig.supylabel('normalized impact (fraction of total asset value)')
